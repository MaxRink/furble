...
## Resume here (kept current; last updated 2026-09-07 12:00)
...
| PR | head | state | next |
| --- | --- | --- | --- |
| #245 stale-bond recovery (plan 151) | e3e3872c | rebased on 107a6b96, CI 34/34 green, lane parked | flash e3e3872c (needs a VM firmware build; d5de73ee is stale by 21 files), bench steps 3 to 10 (camera in pairing mode for 3 to 6), merge |
...
| #139 GPS phase 2 (plan 32) | 42904091 | review CHANGES REQUIRED 10:40, fix round running | same-day IMPLAUSIBLE retry never re-arms (compare UTC time not date; same-day fixture), check-doc-tokens predicate, stale comment, gps platform strtoul end check; arm compares the committed date value (emptyrmc, walkback fixtures), IMPLAUSIBLE bounded at 4; rebased onto 107a6b96 (keep-both resolutions repaired); on approval plus green CI merge, GPS gate owed post-merge |
...
| #291 conn-saver idle coverage test (plan 163 deviation, from plan 151) | 94994a81 | review + CI running | test-only cherry-pick of #245 e3e3872c; the Camera.cpp floor 73.75 is timing-dependent (master 74.75, #139 72.17, lost lines all in requestConnProfile); merge first, then #139/#48/#65 rebase over it |
...
Update 2026-09-06 09:30: #139 review not approved (TSAN race: unlocked TinyGPS accessor that writes; three red CI jobs; unsound ephemeris staleness guard), fix round running with a bundle push via the host. #65 review not ready (the Motion Adaptive toggle resets the receiver through reloadSetting/enable; the 10 Hz timer is invisible to the power gate); fix round running. Order change: #48 lands before #65, and #65 consumes IMU::MotionSource. New order after #265: #47, #139, #45, #48, #65. Issues: #283 (restart re-exec segfault sighting), #284 (restart-persist 5 percent flake).

Update 2026-09-06 10:30: #45 review asks for a provisioning live-apply case and corrects the power table (the model bills any timer fire as a full 5 ms quantum at mcu_80; issue #285). Fix rounds running on #45, #47, #139, #65; #48 building. With the VM idle, two sim/test lanes started: fix/sim-scheduler-visibility (issues #279 and #284, plus a SIGSEGV dump for #283) and fix/coverage-empty-profiles (#277); both hand the coordinator bundles to push from the host. Wire-id collisions among other open PRs noted on #280 (#90/#75 both 62; #59 on 43).

Update 2026-09-06 12:00: #48 ready at d283acff (wire id 74; engine wired; BMI270 feature access rewritten per the Bosch SensorAPI; MotionSource is the shared motion API), review held until VM load drops. #47 at 0bbad1e9 and #45 at 188b603c in delta reviews. Load incident: four lanes built at full parallelism (ninja and coverage.py defaults) despite the -j2 brief; the #47 reviewer build was killed and every lane told to pass -j2 to every tool explicitly (rule recorded in memory).

Update 2026-09-06 12:30: the Claude session credential expired ("Not logged in"); lanes for the #47 delta review, #277, #45 and #139 stopped mid-work and resume from their transcripts after /login (#139 was committing its fix round and building firmware; #45 owes a libdbus-scoped TSAN suppression commit and four nits from its approved delta review; the coverage lane was at its Python tests). #45 delta review approved 188b603c; only the eight-step hardware gate remains. #48 review not yet dispatched (load, then login).
...
Update 2026-09-06 16:30: #48 review found the interrupt path cannot work as written (M5Unified leaves the BMI270 data-ready mapping on INT1 so arming pulses the wake line at ~100 Hz; the M5PM1 IRQ latch is never cleared after a wake; MPU6886 wake status is clear-on-read and stolen by every IMU page read; disarm does not restore M5Unified registers; single-slot callback for a shared API); fix round running with a bundle. Issue #288: ProvisionTLV schema misses shipped ids 43 and 46; every backlog PR adds its row (#139 and #273 confirmed). Reviews still running: #286, #287, #65 delta, #139 delta; fix rounds: #47 (schema rows), #45 (suppression and nits), #48.

Update 2026-09-06 17:00: #65 delta approved at 232d707a; it stays parked for the rebase onto #48 (read MotionSource::state() only; #48 arms once at boot; the 5 s self-calibrating window does not survive #48 memoryless backend, decision recorded in plan 18) and the six-step hardware run. Note on review convention: verdicts are posted as the first line of a comment because GitHub refuses --approve on own PRs; independence comes from the separate reviewer lane.
...
Update 2026-09-07 00:00: #139 delta review found two more blockers (raw RMC count not checksum-valid or fragment-safe because the 256-byte UART buffer splits bursts; IMPLAUSIBLE receiver time treated as final; the sim receiver clock rewinds across restart); fix round running. #273 triage: its red checks are 19 Settings > Display overlap asserts that hinge on the user narrow-panel ruling plus a clang-format fix now in (head 188a7e9a). #48 delta review still running. Three sightings of #283 so far, all power-off-state-matrix under the coverage job.

Update 2026-09-07 00:30: the VM rebooted at 16:22 (it has rebooted once per session day); everything on disk survived and the three lanes with in-flight VM work (#48 delta review, #139 fix round, #273 captures) were told to verify state and continue. ~/wt holds 46 worktrees, most from finished lanes; prune when the wave settles.
