...
`e2e/level-overflow.txt`, `e2e/imu-gesture-shutter.txt`, and
`e2e/redraw-steady.txt`. Gesture settings can be seeded with `imu_wake` (0..3)
and `imu_trigger` (boolean); `imu.accel` drives the shared detector seam.
