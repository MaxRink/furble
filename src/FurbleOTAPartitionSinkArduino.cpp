...
bool ArduinoPartitionTarget::begin(uint32_t imageSize, uint32_t partitionSize) {
  abort();
...
size_t ArduinoPartitionTarget::write(uint32_t offset, const uint8_t *data, size_t length) {
...
bool ArduinoPartitionTarget::matches(uint32_t offset, const uint8_t *data, size_t length) {
  if (!m_Started || m_Ended || (data == nullptr) || (length == 0) || (offset > m_NextOffset)
...
bool ArduinoPartitionTarget::end() {
