...
bool MQTT::shutdownForSimulator(uint32_t timeout_ms) {
  m_SimulatorShutdownComplete.store(false);
  m_SimulatorShutdownDeadlineReached.store(false);
  m_SimulatorShutdownRequested.store(true);
  disconnect();
  if (m_Task == nullptr) {
    m_SimulatorShutdownRequested.store(false);
    m_SimulatorShutdownDeadlineReached.store(false);
    m_SimulatorShutdownComplete.store(true);
    return true;
  }
  xTaskNotifyGive(m_Task);

  const auto started = std::chrono::steady_clock::now();
  const auto grace = std::chrono::milliseconds(std::min(timeout_ms, OFFLINE_ACK_TIMEOUT_MS));
  const auto grace_deadline = started + grace;
  const auto timeout_deadline = started + std::chrono::milliseconds(timeout_ms);
  std::unique_lock<std::mutex> lock(m_Mutex);
  for (;;) {
    if (m_SimulatorShutdownComplete.load()) {
      return true;
    }
    ...
    if (!m_SimulatorShutdownDeadlineReached.load() && (now >= grace_deadline)) {
      // The owner normally wakes on the broker's PUBACK. If the virtual clock
      // is no longer advancing because UI teardown has begun, this real-time
      // notification still lets it apply the bounded graceful-stop timeout.
      lock.unlock();
      m_SimulatorShutdownDeadlineReached.store(true);
      xTaskNotifyGive(m_Task);
      lock.lock();
      continue;
    }
    const auto wake_at = m_SimulatorShutdownDeadlineReached.load()
                             ? timeout_deadline
                             : std::min(grace_deadline, timeout_deadline);
    m_SimulatorShutdownCondition.wait_until(lock, wake_at);
  }
}
#endif
...
bool MQTT::isConfigured(void) const {
  return Settings::load<Settings::MQTT>()
         && Settings::validMQTTString(Settings::MQTT_URI, Settings::load<Settings::MQTT_URI>())
         && Settings::validMQTTString(Settings::MQTT_USER, Settings::load<Settings::MQTT_USER>())
         && Settings::validMQTTString(Settings::MQTT_PASS, Settings::load<Settings::MQTT_PASS>())
         && Settings::validMQTTString(Settings::MQTT_BASE, Settings::load<Settings::MQTT_BASE>());
}
...
void MQTT::task(void) {
  while (true) {
    taskStep();
    ulTaskNotifyTake(pdTRUE, taskWaitTicks());
  }
}

TickType_t MQTT::taskWaitTicks(void) const {
  const uint64_t now = nowMs();
  uint64_t wait_ms = TASK_PERIOD_MS;
  if (m_HoldActive && (m_HoldDeadlineMs != 0)) {
    if (m_HoldDeadlineMs <= now) {
      return 0;
    }
    wait_ms = std::min(wait_ms, m_HoldDeadlineMs - now);
  }
  if (m_IntervalRunning && (m_IntervalNextMs != 0)) {
    if (m_IntervalNextMs <= now) {
      return 0;
    }
    wait_ms = std::min(wait_ms, m_IntervalNextMs - now);
  }
  if (m_OfflineStopPending && (m_OfflineDeadlineMs != 0)) {
    if (m_OfflineDeadlineMs <= now) {
      return 0;
    }
    wait_ms = std::min(wait_ms, m_OfflineDeadlineMs - now);
  }
  return pdMS_TO_TICKS(static_cast<TickType_t>(std::max<uint64_t>(wait_ms, 1)));
}

void MQTT::taskStep(void) {
  if (m_EventQueue == nullptr) {
    m_EventQueue = xQueueCreate(EVENT_QUEUE_LENGTH, sizeof(queued_event_t));
    if (m_EventQueue == nullptr) {
      ESP_LOGE(MQTT_LOG_TAG, "Failed to create MQTT event queue.");
      return;
    }
  }
  ...
  if (reload) {
    // No event copied from the old client may survive into the replacement
    // session. stopClient() may wait for the retained offline PUBACK.
    clearQueuedEvents();
    stopClient();
  }
  ...
  if (!m_Connected.load() && !m_OfflineStopPending) {
    releaseOwnedActuators();
  }
  queued_event_t event;
  for (size_t processed = 0; processed < EVENT_QUEUE_LENGTH; processed++) {
    processDueTimers();
    if (xQueueReceive(m_EventQueue, &event, 0) != pdTRUE) {
      break;
    }
    processEvent(event);
  }
  ...
  // All stop callers are asynchronous: service a pending graceful stop on
  // every owner iteration before loading replacement topics or starting a
  // new client. Never hold a lifecycle lock across stop/destroy.
  if (m_OfflineStopPending) {
    stopClient();
    if (m_OfflineStopPending) {
      return;
    }
...
bool MQTT::enqueueEvent(const queued_event_t &event) {
  if ((m_EventQueue == nullptr) || (xQueueSend(m_EventQueue, &event, 0) != pdTRUE)) {
    ESP_LOGW(MQTT_LOG_TAG, "Dropping MQTT callback event: queue full or unavailable.");
    return false;
  }
  xTaskNotifyGive(m_Task);
  return true;
...
void MQTT::queueLifecycleEvent(esp_mqtt_client_handle_t client, int32_t event_id) {
  {
    const std::lock_guard<std::mutex> lock(m_LifecycleMutex);
    m_LifecycleClient = client;
    m_LifecycleEvent = event_id;
    m_LifecyclePending = true;
  }
  xTaskNotifyGive(m_Task);
}

void MQTT::processLifecycleEvents(void) {
  esp_mqtt_client_handle_t client = nullptr;
  int32_t event_id = 0;
  {
    const std::lock_guard<std::mutex> lock(m_LifecycleMutex);
    if (!m_LifecyclePending) {
      return;
    }
    client = m_LifecycleClient;
    event_id = m_LifecycleEvent;
    m_LifecyclePending = false;
  }

  if ((client == nullptr) || (event_id == 0)) {
    return;
  }
  queued_event_t event;
  event.client = client;
...
void MQTT::clearQueuedEvents(void) {
  queued_event_t event;
  while ((m_EventQueue != nullptr) && (xQueueReceive(m_EventQueue, &event, 0) == pdTRUE)) {
  }
  const std::lock_guard<std::mutex> lock(m_LifecycleMutex);
  m_LifecyclePending = false;
  m_LifecycleClient = nullptr;
  m_LifecycleEvent = 0;
}

void MQTT::processEvent(const queued_event_t &event) {
  if (event.type == queued_event_type_t::MQTT) {
    handleEvent(event);
  }
}

void MQTT::processDueTimers(void) {
  const uint64_t now = nowMs();
  if (m_HoldActive && (m_HoldDeadlineMs != 0) && (now >= m_HoldDeadlineMs)) {
    releaseHold();
  }
  if (m_IntervalRunning && (m_IntervalNextMs != 0) && (now >= m_IntervalNextMs)) {
    intervalStep();
  }
}

void MQTT::servicePendingReleases(void) {
  if (m_ShutterReleasePending && m_ShutterOwned && sendCommand(Control::CMD_SHUTTER_RELEASE)) {
    m_ShutterReleasePending = false;
    m_ShutterOwned = false;
    publishShutter("idle");
  }
  if (m_FocusReleasePending && m_FocusOwned && sendCommand(Control::CMD_FOCUS_RELEASE)) {
...
void MQTT::stopClient(void) {
  ...
  if (client == nullptr) {
    ...
    m_OfflineStopPending = false;
    m_OfflineAcked = false;
    m_OfflineMessageId = -1;
    m_OfflineDeadlineMs = 0;
#if defined(FURBLE_SIM_MQTT)
    if (m_SimulatorShutdownRequested.exchange(false)) {
      m_SimulatorShutdownDeadlineReached.store(false);
      m_SimulatorShutdownComplete.store(true);
      m_SimulatorShutdownCondition.notify_all();
    }
#endif
    return;
  }
  ...
  if (m_OfflineStopPending) {
    const bool simulator_shutdown =
#if defined(FURBLE_SIM_MQTT)
        m_SimulatorShutdownRequested.load() && m_SimulatorShutdownDeadlineReached.load();
#else
        false;
#endif
    if (!simulator_shutdown && !m_OfflineAcked && (nowMs() < m_OfflineDeadlineMs)) {
      return;
    }
  } else if (m_Connected.load()) {
    int message_id = -1;
    if (publish(m_StatusTopic, "offline", 1, true, &message_id)) {
      m_OfflineStopPending = true;
      m_OfflineAcked = false;
      m_OfflineMessageId = message_id;
      m_OfflineDeadlineMs = nowMs() + OFFLINE_ACK_TIMEOUT_MS;
      return;
    } else {
      ESP_LOGW(MQTT_LOG_TAG, "MQTT graceful offline enqueue failed.");
    }
  }
...
void MQTT::eventHandler(void *handler_arg,
  ...
  const auto *source = static_cast<const esp_mqtt_event_t *>(event_data);
  if (source == nullptr) {
    return;
  }

  MQTT::queued_event_t event;
  event.type = MQTT::queued_event_type_t::MQTT;
  event.client = source->client;
  event.event_id = source->event_id;
  event.qos = source->qos;
  event.retain = source->retain;
  event.dup = source->dup;
  event.message_id = source->msg_id;
  if (source->error_handle != nullptr) {
    event.error_type = source->error_handle->error_type;
  }

  if ((source->event_id == MQTT_EVENT_CONNECTED) || (source->event_id == MQTT_EVENT_DISCONNECTED)) {
    mqtt->queueLifecycleEvent(source->client, source->event_id);
    return;
  }

  if (source->event_id == MQTT_EVENT_DATA) {
    if ((source->topic == nullptr) || (source->data == nullptr) || (source->topic_len <= 0)
        || (source->data_len < 0) || (source->total_data_len < 0)
        || (source->topic_len > static_cast<int>(MQTT::MAX_COMMAND_TOPIC_LENGTH))
        || (source->data_len > static_cast<int>(MQTT::MAX_COMMAND_PAYLOAD_LENGTH))
        || (source->total_data_len > static_cast<int>(MQTT::MAX_COMMAND_PAYLOAD_LENGTH))) {
      ESP_LOGW(MQTT_LOG_TAG, "Ignoring invalid or oversized MQTT command.");
      return;
    }
    event.current_data_offset = source->current_data_offset;
    event.total_data_len = source->total_data_len;
    event.topic_len = source->topic_len;
    event.data_len = source->data_len;
    std::memcpy(event.topic, source->topic, static_cast<size_t>(source->topic_len));
    std::memcpy(event.data, source->data, static_cast<size_t>(source->data_len));
    event.topic[source->topic_len] = '\0';
    event.data[source->data_len] = '\0';
  }

  mqtt->enqueueEvent(event);
}

void MQTT::handleEvent(const queued_event_t &event) {
  // Compare the copied identity before using any event-owned client pointer.
  // A stopped client may still deliver a callback after stop/destroy returns.
  if ((event.client == nullptr) || (event.client != m_Client)) {
    return;
  }
  if (m_OfflineStopPending && (event.event_id != MQTT_EVENT_PUBLISHED)) {
    return;
  }
  ...
  switch (event.event_id) {
    ...
    case MQTT_EVENT_DISCONNECTED:
    {
      ...
      if (!manual_disconnect && !stop_client) {
        m_ConnectFailures++;
    ...
    case MQTT_EVENT_ERROR:
      ESP_LOGW(MQTT_LOG_TAG, "MQTT error type %d.", static_cast<int>(event.error_type));
      break;

    case MQTT_EVENT_PUBLISHED:
      if (m_OfflineStopPending && (event.message_id == m_OfflineMessageId)) {
        m_OfflineAcked = true;
#if defined(FURBLE_SIM_MQTT)
        ESP_LOGI(MQTT_LOG_TAG, "MQTT simulator offline PUBACK received.");
#endif
      }
      break;
...
void MQTT::handleData(const queued_event_t &event) {
  if (!m_Connected.load()) {
    return;
  }
  if ((event.topic_len <= 0) || (event.data_len < 0) || (event.total_data_len < 0)
      || (event.topic_len > static_cast<int>(MAX_COMMAND_TOPIC_LENGTH))
      || (event.data_len > static_cast<int>(MAX_COMMAND_PAYLOAD_LENGTH))
      || (event.total_data_len > static_cast<int>(MAX_COMMAND_PAYLOAD_LENGTH))) {
    ESP_LOGW(MQTT_LOG_TAG, "Ignoring invalid or oversized MQTT command.");
    return;
  }
  ...
  if ((std::memchr(event.topic, '\0', static_cast<size_t>(event.topic_len)) != nullptr)
      || (std::memchr(event.data, '\0', static_cast<size_t>(event.data_len)) != nullptr)) {
    ESP_LOGW(MQTT_LOG_TAG, "Ignoring MQTT command containing NUL.");
    return;
  }
  ...
  if (event.retain && startsWith(topic, m_RootTopic + "/cmd/")) {
    publishError("retained MQTT command rejected");
    return;
  }
...
void MQTT::handleCommand(const std::string &topic, const std::string &payload) {
  ...
  if (topic == m_ShutterCommandTopic) {
    if (value == "press") {
      if (m_ShutterReleasePending || m_FocusReleasePending) {
        publishError("shutter press rejected: actuator release pending");
        return;
      }
      releaseHold();
      if (m_ShutterReleasePending || m_FocusReleasePending
          || !sendCommand(Control::CMD_SHUTTER_PRESS)) {
  ...
  if (topic == m_FocusCommandTopic) {
    ...
    if ((command == Control::CMD_FOCUS_PRESS)
        && (m_ShutterReleasePending || m_FocusReleasePending)) {
      publishError("focus press rejected: actuator release pending");
      return;
    }

    if (!sendCommand(command)) {
      if ((command == Control::CMD_FOCUS_RELEASE) && m_FocusOwned) {
        m_FocusReleasePending = true;
      }
      publishError("focus command rejected");
    } else if (command == Control::CMD_FOCUS_PRESS) {
      m_FocusOwned = true;
    } else {
      m_FocusReleasePending = false;
      m_FocusOwned = false;
  ...
  if (topic == m_ConnectCommandTopic) {
    uint8_t cameraId = CameraListProtocol::INDEX_ID_ALL;
    if (!value.empty() && (value != "all")) {
      uint32_t parsed = 0;
      if (!parseUnsigned(value, parsed) || (parsed > UINT8_MAX)
          || (parsed == CameraListProtocol::INDEX_ID_INVALID)
          || (parsed == CameraListProtocol::INDEX_ID_ALL)) {
        publishError("camera id is invalid");
        return;
      }
      cameraId = static_cast<uint8_t>(parsed);
    }
    if (!UI::sendRequest(UI::Request::CONNECT_SAVED, cameraId)) {
      publishError("connect request rejected");
      return;
    }
    publishCameras(true);
    return;
  ...
  if (topic == m_DisconnectCommandTopic) {
    if (!UI::sendRequest(UI::Request::DISCONNECT, 0)) {
      publishError("disconnect request rejected");
      return;
    }
    publishCameras(true);
    return;
...
bool MQTT::sendHold(uint32_t duration_ms) {
...

  {
    const std::lock_guard<std::mutex> lock(m_Mutex);
    m_HoldActive = true;
    m_HoldDeadlineMs = nowMs() + std::max<uint32_t>(duration_ms, 1);
  }
  m_ShutterOwned = true;
...
void MQTT::holdTimerCallback(void *arg) {
  // Timer callbacks do not touch MQTT state. The owner checks the deadline;
  // no callback event is queued, so a full MQTT queue cannot strand a hold.
  auto *mqtt = static_cast<MQTT *>(arg);
  if (mqtt != nullptr) {
    xTaskNotifyGive(mqtt->m_Task);
  }
}
...
void MQTT::releaseOwnedActuators(void) {
  releaseHold();
  stopInterval();
  if (m_ShutterOwned) {
    if (sendCommand(Control::CMD_SHUTTER_RELEASE)) {
  ...
  if (m_FocusOwned) {
    if (sendCommand(Control::CMD_FOCUS_RELEASE)) {
      m_FocusReleasePending = false;
      m_FocusOwned = false;
    } else {
      m_FocusReleasePending = true;
...
void MQTT::startInterval(void) {
  if (m_ShutterReleasePending || m_FocusReleasePending) {
    publishError("interval start rejected: actuator release pending");
    return;
  }
  auto &control = Control::getInstance();
  if (control.getState() != Control::STATE_ACTIVE) {
...
void MQTT::intervalTimerCallback(void *arg) {
  // Timer callbacks do not touch MQTT state. Interval state is deadline driven
  // in processDueTimers(), avoiding dropped timer queue events.
  auto *mqtt = static_cast<MQTT *>(arg);
  if (mqtt != nullptr) {
    xTaskNotifyGive(mqtt->m_Task);
  }
...
void MQTT::publishCameras(bool force) {
  ...
  const auto cameras = CameraList::savedSnapshot();
  const std::string payload = makeCamerasPayload(cameras);
  ...
  for (const auto &camera : cameras) {
    ...
    if (cameraId == CameraListProtocol::INDEX_ID_INVALID) {
      continue;
    }
    int8_t cachedRssi = 0;
    const bool target = control.getTargetState(camera.get(), cachedRssi);
    Control::target_status_t status = {
        std::to_string(cameraId),
        camera->getName(),
        camera->getType(),
        target && camera->isConnected(),
        static_cast<uint8_t>(target ? camera->getConnectProgress() : 0),
        static_cast<int16_t>(cachedRssi)};
...
std::string MQTT::makeCamerasPayload(const std::vector<std::shared_ptr<Camera>> &cameras) const {
  ...
  for (const auto &camera : cameras) {
    ...
    int8_t cachedRssi = 0;
    const bool target = control.getTargetState(camera.get(), cachedRssi);
    const bool connected = target && camera->isConnected();
    const uint8_t progress = target ? camera->getConnectProgress() : 0;
    const std::string id = std::to_string(cameraId);
    ...
    cJSON_AddNumberToObject(item, "rssi", rssiBucket(cachedRssi));
...
void MQTT::publishDiscovery(void) {
  ...
  for (const auto &camera : cameras) {
    ...
    const std::string camera_id = std::to_string(cameraId);
