...
bool NimBLEClient::completeConnectEstablished() {
    ...
    m_connectCallbackPending = false;
    ble_npl_callout_stop(&m_connectEstablishedTimer);
    auto pTaskData = extractTaskData(); // claim before callback to prevent other handlers from releasing
...
/**
 * @brief Atomically take ownership of the task data of a waiting task.
 * @return The task data pointer if one was set, nullptr otherwise.
 * @details Only the context that claims the task data may release it. This prevents
 * a waiting task that timed out from destroying the task data while another context
 * still references it, and prevents a second context from releasing it again.
 */
...
int NimBLEClient::handleGapEvent(struct ble_gap_event* event, void* arg) {
    NimBLEClient*   pClient   = (NimBLEClient*)arg;
    int             rc        = 0;
    NimBLETaskData* pTaskData = nullptr; // claimed by each event branch that will release a waiting task
    ...
    switch (event->type) {
        case BLE_GAP_EVENT_DISCONNECT: {
            ...
            // Claim the task data of any waiting task now, the client instance may be
            // deleted below so it cannot be read from the client after this point.
            pTaskData = pClient->extractTaskData();
