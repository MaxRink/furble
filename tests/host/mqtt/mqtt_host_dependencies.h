...
class Control {
 public:
  ...
  bool getTargetState(const Camera *camera, int8_t &rssi) const {
    ...
    for (const auto &target : m_Targets) {
      if (target.id == camera->getID()) {
        rssi = static_cast<int8_t>(target.rssi);
        return true;
      }
    }
    return false;
  ...
  static void failNextRelease(void) { m_FailNextRelease = true; }
 ...
 private:
  ...
  inline static bool m_FailNextRelease = false;
};
...
class Settings {
 public:
  ...
  static bool validMQTTString(type_t type, const std::string &value) {
    return (type == MQTT_URI || type == MQTT_BASE) ? !value.empty() : true;
  }
};
