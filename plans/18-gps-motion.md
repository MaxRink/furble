...
This PR is stacked on PR48 (`feat/20-hw-motion`) and rebased onto af3f982a,
which is itself on master 35c0f4b8. The
...
The union merge that this second rebase was expected to need is done, and it
was four files as forecast, plus two the forecast missed:

- `SETTING_SCHEMAS` in `lib/furble/protocol/ProvisionTLV.cpp` now runs 46, 66,
  67, 68, 72, 73, 74 in ascending order. Wire id 66 did not move and its five
  golden fixtures regenerate byte identical, which is the check that the id was
  not quietly reassigned.
- `include/CLAUDE.md` needed no merge at all. Master now carries the issue #280
  reservation table and it already lists `#65 | gps_motion | 66`, so this PR's
  older prose about the same reservation was dropped rather than merged.
- The `type_t` enum in `include/FurbleSettings.h` and its copy in
  `tests/host/companion/doubles/FurbleSettings.h` are ordered GPS_HOLD,
  GPS_EXTRAP, GPS_MOTION in both. They have to stay identical or the companion
  doubles map settings to the wrong ids, so they are resolved as one edit.
- `provision_apply_test`'s bundle order is the applied order, so adding wire id
  66 to the bundle means `appliedIds` becomes `{1, 26, 33, 27, 46, 72, 73, 66}`.
  Merging the bundle without the expectation is a silent failure.
- Two the forecast missed. `sim/scenarios/bughunt/*notouch*.txt` carried a
  `nav motion_engine` block that no longer resolves, because PR48 moved the
  Motion Engine roller onto PR45's Gestures page and deleted the dedicated one.
  And `sim/build.sh` reuses its object directory, so a build directory from
  before this rebase links a stale TinyGPS++ object against the new
  `sim/clock.cpp` and fails with `multiple definition of millis()`. Wipe the
  sim build directory after a rebase that crosses a base change.

Restacking this branch always uses `git rebase --onto <new base> <old base>`.
PR48 is itself rebased between rounds, so its commits change SHA; a plain
`git rebase <new base>` sees the old ones as unmerged and tries to replay
PR48's whole series on top of itself, producing dozens of conflicts in files
this PR never touched. Naming the old base explicitly replays only the four
commits that belong here. The signal that it went wrong is a conflict in a
file outside the list above.

Master also carries `testEverySettingHasASchemaRow`, the class-wide guard that
...
`KNOWN_MISSING`. Wire id 66 has its row and the test passes with it. Wire id 46
has since gained a row too, so the only gap left in that list is 43.
