...
The serial uploader now classifies dependency, port, and handshake failures
separately. A system Python without pyserial is guided to the pinned
...
reports that no upload was started.
...
- `tests/test_flash_prepare.py` covers all preflight result classes, including
  missing pyserial and unopened ports, and verifies that dry runs do not claim
  to have started an upload. It also checks that uploads use the normal build
  target and provide the required build identity environment.
