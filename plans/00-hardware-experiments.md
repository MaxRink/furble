...
| 2026-08-16 | combined + XTAL UART clock fix | StickS3 + GPS v1.1 | B part 1 | With a fresh fix (6 sats), the 5 V rail was cut for 60 s via the console. Re-fix took 107.7 s after restore, a full cold start. Reception before and after the test: zero checksum failures with the UART on the XTAL clock. | No backup supply on Unit GPS v1.1. Rail cycling costs a cold start every time. PR15 defaults to $PCAS12 standby duty cycling; rail cycling only becomes viable with the plan 32 ephemeris cache. The XTAL clock fix is hardware verified. |
