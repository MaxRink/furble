...
- secureConnection() moves the m_pTaskData assignment into the retry loop
  (the claimer nulls it on release, so the PINKEY retry has to rearm it) and
  claims back after each iteration in case startSecurity failed and nobody
  will release.
...
- The pinned 2.5.0 release is vendored at components/esp-nimble-cpp with
  docs/, examples/ and CHECKSUMS.json stripped (872 KB). Provenance is the
