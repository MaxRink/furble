...
| Developer console | The UI-side `UI::Request` handlers in `src/FurbleUI.cpp`, built here because `FURBLE_CONSOLE` is defined in both `sim/CMakeLists.txt` and `sim/build.sh` | `src/FurbleConsole.cpp` is not in the SDL build: it is the serial transport and the command parser, both driven end to end by `tests/host/console_commands_test.cpp` against the real ESP-IDF console API. `sim/FurbleConsoleSim.cpp` supplies the entry points other firmware calls. Scenarios post a request with `action console <request> [arg]`, which calls production `UI::sendRequest()`, and read the answer through the `console.<key>` assert namespace fed by `UI::consolePrint`. The LVGL invalidation logger in `UI::init()` is the one thing `FURBLE_CONSOLE` brings which the simulator does not take: it is a device diagnostic that runs on every invalidation, so it is `#if defined(FURBLE_CONSOLE) && !defined(FURBLE_SIM)` rather than a new per-invalidate cost in every scenario and power regression run. |
...
- Scenario parsing is a pre-runtime gate: every verb has strict arity and
  ...
  `INVALID`, and malformed direct calls fail closed. Plain `action` lines
  require `APPLIED`; intentional `VALID_NO_EFFECT` or `UNAVAILABLE` outcomes
  must use `action expect no-effect ...` or `action expect unavailable ...`.
  The valid root aliases
