...
int main(int argc, char **argv) {
  ...
  // A scripted, UI-triggered, interactive, or fuzz restart reboots the
  // simulated device by re-executing this binary. The re-exec waits until here
  // so the reboot follows the same orderly shutdown a clean exit takes: every
  // task stopped and joined, the panel closed. Any failure raised between the
  // request and this point wins over the reboot, so a broken scenario reports
  // its result instead of booting again.
