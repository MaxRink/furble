...
if [ -z "$timeout_digits" ] || [ "$START_TIMEOUT" -lt 1 ] || [ "$START_TIMEOUT" -gt 3600 ]; then
  echo "FURBLE_SIM_DEEP_SLEEP_START_TIMEOUT must be an integer from 1 to 3600" >&2
  exit 2
fi
...
export FURBLE_SIM_DEEP_SLEEP_PREFS="$PREFS"
...
unset FURBLE_SIM_DEEP_SLEEP_PREFS
unset FURBLE_SIM_PRESERVE_PREFS
unset FURBLE_SIM_PREFS
