...
struct RequestState {
  ...
  ~RequestState() { if (done != nullptr) vSemaphoreDelete(done); }
};
struct RequestResult {
  RequestState *state = nullptr;
  RequestResult() noexcept : state(new (std::nothrow) RequestState()) {
    if (state != nullptr && state->done == nullptr) { state->release(); state = nullptr; }
  }
  ~RequestResult() { if (state != nullptr) state->release(); }
